...
module CalculoAnticipos # <-- Asegúrate de que esté escrito exactamente así (C y A mayúsculas)
  extend ActiveSupport::Concern

  class_methods do
    def generar_anticipos_x500_segundo_plano
      ...
      # 3. Iteración y creación de registros bajo transacción segura
      self.transaction do
        totales_por_cargo.each_with_index do |cargo, index|
          ...
          if (procesados % 20).zero? || (procesados == total_registros)
            ...
            # Transmitir al navegador en tiempo real
            Turbo::StreamsChannel.broadcast_replace_to(
              'nomina_channel',
              target: 'progreso_cargos_anticipos',
      ...
      Turbo::StreamsChannel.broadcast_replace_to(
        'nomina_channel',
        target: 'progreso_cargos_anticipos', # <-- Cambiado
